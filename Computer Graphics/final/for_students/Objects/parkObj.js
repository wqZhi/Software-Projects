...
export class GrCarousel extends GrObject {
  ...
  constructor(params = {}) {
    ...
    for (let i = 0; i < num_poles; i++) {
      ...
      //adding horses
      let horse = loader.loadAsync("./Objects/horse.obj");
  ...
  stepWorld(delta, timeOfDay) {
    ...
    // set horses animation
    for (let i = 0; i < this.num; i++) {
      // let horse = this.platform_group.getObjectByName(`horse${i}`);
      if (this.horses[i]) {
        const t = (this.time + 0.1 * i) % 1;

        if (t < 0.1 || t > 0.9) this.horses[i].position.y = 0.2;
        else {
          this.horses[i].position.y = 0.2 + 5 * (0.16 - (0.5 - t) * (0.5 - t));
        }
      }
    }
 ...
 * @property {number} [size=1]
 */
export class GrSpinningTeapot extends GrObject {
  /**
   * @param {SpinningTeacupsProperties} params
   */
  constructor(params = {}) {
    let spinningTeapot = new T.Group();
    ...
    let base = new T.Mesh(base_geom, base_mat);
    base.translateY(0.25);
    spinningTeapot.add(base);

    let platform_geom = new T.CylinderGeometry(4, 1.8, 0.3, 8, 4);
    let platform_mat = new T.MeshStandardMaterial({
      color: "gray",
      metalness: 0.3,
      roughness: 0.6
    ...
    for (let i = 0; i < num_poles; i++) {
      //adding cups
      let horse = loader.loadAsync("./Objects/teapot.obj");
      horse.then(function(obj) {
    ...
    // note that we have to make the Object3D before we can call
    // super and we have to call super before we can use this
    super(`SpinningTeapot-${SpinningTeapotObCtr++}`, spinningTeapot);
    this.whole_ob = spinningTeapot;
    this.platform = platform_group;
    this.cups = cups;
    ...
    let scale = params.size ? Number(params.size) : 1;
    spinningTeapot.scale.set(scale, scale, scale);
  }
  /**
   * StepWorld method
   * @param {*} delta 
  ...
  stepWorld(delta, timeOfDay) {
    ...
    this.cups.forEach(function(obj) {
      obj.rotateY(0.005 * delta);
    });
  }

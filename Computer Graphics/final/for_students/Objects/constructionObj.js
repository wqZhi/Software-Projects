...
//from wb 07-09-constructionobjects
...
export class GrCrane extends GrObject {
  ...
  stepWorld(delta, timeOfDay) {
    this.swing_angle += 0.001 * delta;
    this.arm.rotation.y = (Math.sin(this.swing_angle) * Math.PI) / 4;
    this.wire.position.x = 2+ (Math.sin(this.swing_angle) * Math.PI) / 3;
  }
...
//     this.arm.position.y = degreesToRadians(paramValues[4]);
//   }
}
...
export class GrExcavator extends GrObject {
  ...
  stepWorld(delta, timeOfDay) {
    this.swing_angle += 0.001 * delta;
    this.cab.rotation.y = (Math.sin(this.swing_angle) * Math.PI) / 4;
    this.arm.rotation.z = (Math.sin(this.swing_angle) * Math.PI) / 8;
    this.forearm.rotation.z = (Math.sin(this.swing_angle) * Math.PI) / 4;
    this.bucket.rotation.z = (Math.sin(this.swing_angle) * Math.PI) / 3;
  }
...
export class GrForkLift extends GrObject {
  ...
  stepWorld(delta, timeOfDay) {
    this.curve = new T.CatmullRomCurve3( [
      new T.Vector3( 5, 0, -12.2),
      new T.Vector3( -16, 0,  -12.2),
      new T.Vector3( -16, 0,  4.8),
      new T.Vector3( 5, 0, 4.8),

     ], true, "catmullrom", 0.15);

  
      this.time += delta / 10001;
    ...
    this.swing_angle += 0.001 * delta;
    this.fork.position.y = 1 + (Math.sin(this.swing_angle) * Math.PI) / 4;
  }
...
export class GrForkLiftCrane extends GrObject {
  ...
  stepWorld(delta, timeOfDay) {
    this.swing_angle += 0.001 * delta;
    this.fork.position.y = 1 + (Math.sin(this.swing_angle) * Math.PI) / 4;
    this.forearm.position.z = 1.2 + (Math.sin(this.swing_angle) * Math.PI) / 4;

...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        try {
            Hashtable<Vertex, Path> nodeTable = new Hashtable<>();
            PriorityQueue<Path> queue = new PriorityQueue<>();
            Hashtable<Vertex, Path> shortestPath = new Hashtable<>();

            Path path = new Path(this.vertices.get(start));
            nodeTable.put(this.vertices.get(start), path);
            queue.offer(path);

            while (!queue.isEmpty()) {
                Path tempPath = queue.poll();
                Vertex tempVertex = tempPath.end;
                shortestPath.put(tempVertex, tempPath);

                // reach to the end of path, then turn
                if (tempVertex.data.equals(end)) {
                    return tempPath;
                }

                // check neighbors
                for (int i = 0; i < tempVertex.edgesLeaving.size(); i++) {
                    Edge neighbor = tempVertex.edgesLeaving.get(i);
                    // if current neighbor already checked in the short path, then continue loop
                    if (shortestPath.containsKey(neighbor.target)) {
                        continue;
                    } else {
                        int totalDistance = tempPath.distance + neighbor.weight;
                        Path p = nodeTable.get(neighbor.target);
                        // if the neighbor is first time to meet
                        if (p == null) {
                            p = new Path(tempPath, neighbor);
                            nodeTable.put(neighbor.target, p);
                            queue.offer(p);
                        }
                        // otherwise, check if neighbor distance is shorter than totalDistance
                        else if (p.distance > totalDistance) {
                            p.distance = totalDistance;
//                            tempPath.dataSequence.add(neighbor.target.data);
//                            p.dataSequence = tempPath.dataSequence;
                            queue.remove(p);
                            queue.add(p);
                        }
                    }

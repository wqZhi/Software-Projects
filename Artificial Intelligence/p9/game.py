...
class Teeko2Player:
    ...
    def make_move(self, state):
        ...
        for row in state:
            for index in row:
                if index != " ":
                    count += 1
        drop_phase = False if count == 8 else drop_phase = True 
    ...
    def game_value(self, state):
        ...
        for row in range(3):
            for col in range(3):
                if state[row][col] != " " and state[row][col] == state[row][col+2] == state[row+2][col] == state[row+2][col+2] and state[row+1][col+1] == ' ':
                    return 1 if state[row][col] == self.my_piece else -1        

...
class LeNet(nn.Module):
    def __init__(self, input_shape=(32, 32), num_classes=100):
        ...
        self.maxPool_conv_1 = nn.MaxPool2d(kernel_size = 2, padding = 0,stride = 2)
        self.conv_2 = nn.Conv2d(in_channels=6, out_channels=16,kernel_size=5,stride=1, padding=0)
        self.maxPool_conv_2 = nn.MaxPool2d(kernel_size = 2, padding = 0,stride = 2)
    ...
    def forward(self, x):
        ...
        x = self.maxPool_conv_1(x)
        shape_dict[1] = list(x.shape)
        
        x = nn.functional.relu(self.conv_2(x))
        x = self.maxPool_conv_2(x)

...
public class TrackingApp {
    ...
    private static void addNewRoom(Scanner in) {
        ...
        try {
            ...
            System.out
                .println("ERROR: Unable to create room " + roomName + " with capacity " + cap);
        }
    }
    ...
    private static void checkPersonIn(Scanner in) {
        ...
        while (personIndex < 0) {
            ...
            else if (personIndex >= 0 && personIndex < people.size() && !people.get(personIndex)
                .isWaiting()) {
                System.out.println(
                    "ERROR: Person " + people.get(personIndex).getName() + " is already in a room");
        ...
        if (personIndex < people.size()) {
            ...
            if (rooms.get(roomIndex).checkIn(people.get(personIndex))) {
                System.out.println(
                    "Successfully checked " + people.get(personIndex).getName() + " into room "
                        + Room.getNames()[roomIndex]);
            }
    ...
    private static void checkPersonOut(Scanner in) {
        ...
        while (personIndex < 0) {
            ...
            if (personIndex == -2) {
                ...
                .isWaiting()) {
                System.out.println(
                    "ERROR: Person " + people.get(personIndex).getName() + " is not in a room");
                personIndex = people.size();
            }
        }
        if (personIndex < people.size()) {
            ...
            if (rooms.get(roomIndex).checkOut(people.get(personIndex))) {
                System.out.println(
                    "Successfully checked " + people.get(personIndex).getName() + " out of room "
                        + Room.getNames()[roomIndex]);
            }
            if (!people.get(personIndex).isWaiting()) {
